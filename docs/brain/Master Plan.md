...
| **2** 🔧 | **The universal field** (2026-09-26: built, reviewed three times; rounds 1–3 merged; rounds 4–5 complete; three explicit acceptance checks **failed** live on AOT: 'notes' query still ranks folder over notes.txt, 'note' misses notebook.txt, 'sample notes budget' lists memo 3.txt. See [[Log/2026-09-26#Round 5]]) | Converters, bangs/custom commands, files + apps from the engine index, clipboard history + secret guard, open files in tabs, search://play media route. **Blockers before shipping:** (1) verify field's live query path matches unit tests (build_result_item / typed_keyword_hits counting); (2) re-verify all three acceptance checks live on AOT; (3) run full release smoke (Settings Add Folder, player next/prev, all three suite passes, bench-field 21/21 on AOT); (4) update docs/brain per CLAUDE.md. | 3–4 wks |
