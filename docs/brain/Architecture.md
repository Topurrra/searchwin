...
| `Shield.cs` | Shields: one `*` request filter decided against a compiled list (`SearchKit.Shields`), the shared and per-site ad-slot stylesheets, tracking tags and redirect wrappers off addresses (`Tab.Tidy`, `NavigationStarting`, Copy Address), the page scripts (YouTube, cookie banners, AMP), per-site pause |
| `ShieldLists.cs` | EasyList + EasyPrivacy: opt-in download from easylist.to, daily check (ETag), compiled blob in `Shields\`, fails open to the built-in 44 domains |
...
- `Shields\`: EasyList/EasyPrivacy text, the compiled `lists.bin` and `lists.json` (tags, dates), only once the full lists are turned on
