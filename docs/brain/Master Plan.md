...
| **3** | **Tools as pages + FFmpeg pack** | Packs framework (Settings › Packs: download, verify, update, remove) and the **FFmpeg pack** (LGPL build; the player plays MKV/AVI/… by remuxing, subtitles; media tools). Tool pages pack by pack: Utilities → Files (file manager) → Privacy → Images → Documents (without Word ⇄ PDF) → Dev → Media → Focus, each restyled and tested | 4–8 wks |
